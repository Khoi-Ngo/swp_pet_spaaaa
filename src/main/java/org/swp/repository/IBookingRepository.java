...
public interface IBookingRepository extends JpaRepository<Booking, Integer> {
    ...
    @Query(value = "SELECT * FROM tbl_booking WHERE pet_id = :petId AND status = :status AND is_deleted = FALSE", nativeQuery = true)
    List<Booking> findByPetIdAndStatus(@Param("petId") Integer petId, @Param("status") String status);

    @Query(value = "SELECT * FROM tbl_booking WHERE pet_id = :petId AND is_deleted = FALSE", nativeQuery = true)
    List<Booking> findByPetId(@Param("petId") Integer petId);

    @Query(value = "SELECT * FROM tbl_booking WHERE shop_id = :shopId AND is_deleted = FALSE", nativeQuery = true)
