...
public interface INominationRepository extends JpaRepository<Nomination, Integer> {
    ...
    @Query(value = "SELECT * FROM tbl_nomination WHERE shop_id = :shopId AND user_id = :userId AND is_deleted = FALSE", nativeQuery = true)
    Nomination findByShopIdAndUserId(@Param("shopId") int shopId, @Param("userId") int userId);

    @Query(value = "SELECT * FROM tbl_nomination WHERE service_id = :serviceId AND user_id = :userId AND is_deleted = FALSE", nativeQuery = true)
